...
const getTaskByDateRange = (tasks, dateFrom, dateTo) => {
...
const createStatisticsTemplate = (tasks, dateFrom, dateTo) => {
  ...
  const tasksCount = getTaskByDateRange(tasks, dateFrom, dateTo).length;
...
export default class Statistics extends AbstractSmartComponent {
  ...
  getTemplate() {
    return createStatisticsTemplate(this._tasks.getTasks(), this._dateFrom, this._dateTo);
  }
  ...
  show() {
    super.show();
    this._renderCharts(this._tasks, this._dateFrom, this._dateTo);
  }
  ...
  _applyFlatpickr(element) {
    ...
    this._flatpickr = flatpickr(element, {
      ...
      onChange: (dates) => {
        if (dates.length === 2) {
          this._rerender(this._tasks, dates[0], dates[1]);
        }
      }
    });
  }

  _rerender(tasks, dateFrom, dateTo) {
    this._tasks = tasks;
    this._dateFrom = dateFrom;
    this._dateTo = dateTo;
    this.rerender();
